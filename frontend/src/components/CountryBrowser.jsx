...
export default function CountryBrowser({ data, loading }) {
  ...
  const categories = ['all', ...new Set(countries.map(c => c.category))];
  ...
  const filteredCountries = useMemo(() => {
    ...
    // Sort
    if (sortBy === 'loss') {
      result.sort((a, b) => Math.abs(b.daily_loss) - Math.abs(a.daily_loss));
    } else if (sortBy === 'gdp') {
      result.sort((a, b) => b.gdp_slowdown_percent - a.gdp_slowdown_percent);
  ...
  const getCountryColor = (category) => {
    if (category.includes('Direct')) return 'from-red-500 to-red-600';
    if (category.includes('Importer')) return 'from-orange-500 to-orange-600';
    if (category.includes('Exporter')) return 'from-green-500 to-green-600';
    return 'from-blue-500 to-blue-600';
  };

  const getCategoryBadge = (category) => {
    if (category.includes('Direct')) return 'bg-red-900 text-red-200 border-red-500';
    if (category.includes('Importer')) return 'bg-orange-900 text-orange-200 border-orange-500';
    if (category.includes('Exporter')) return 'bg-green-900 text-green-200 border-green-500';
        ...
        <AnimatePresence>
          {filteredCountries.map((country, idx) => (
            <motion.div
              ...
              whileHover={{ y: -5 }}
              className={`bg-gradient-to-br ${getCountryColor(country.category)} p-0.5 rounded-lg cursor-pointer group hover:shadow-lg hover:shadow-orange-500/50 transition-all`}
            >
              <div className="bg-slate-800 p-6 h-full rounded-lg">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`text-xs font-semibold px-2 py-1 rounded border ${getCategoryBadge(country.category)}`}>
                    {country.category.split(' ')[country.category.split(' ').length - 1]}
                  </span>
                </div>
                ...
                <div className="space-y-3 mb-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-orange-400">
                      ${Math.abs(country.daily_loss).toFixed(0)}M
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-red-400">
                      ${(country.cumulative_loss / 1000).toFixed(1)}B
                    </p>
                  </div>

                  <div>
                    <p className="text-gray-400 text-sm">GDP Impact per Day</p>
                    <p className={`text-lg font-bold ${country.gdp_slowdown_percent > 0 ? 'text-red-400' : 'text-green-400'}`}>
                      {country.gdp_slowdown_percent > 0 ? '-' : '+'}{Math.abs(country.gdp_slowdown_percent).toFixed(3)}%
                    </p>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-slate-700">
                  <p className="text-xs text-gray-400 line-clamp-2">
                    <strong>Impact:</strong> {country.energy_impact}
                  </p>
                </div>
              </div>
            </motion.div>
